...
def collect_request_stats_decorator(func, app_id, app_secret, another_stats_funcs=[]):
    def wrappe(self: CollectStatsMixin, request, *args, **kwargs):
        try:
            app_tokens = AuthRequester().app_get_token(app_id, app_secret, get_token_from_request(request))
